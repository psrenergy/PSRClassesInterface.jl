function test_api(data_path::String)
    ...
    for (collection, series_list) in series_data
        for (attribute, new_value) in series_list
            ...
            @test PSRI.Tables.getcolumn(new_value_st, 1) ==
                  PSRI.Tables.getcolumn(value_set, 1)
            @test PSRI.Tables.getcolumn(new_value_st, keys(new_value_st)[1]) ==
                  PSRI.Tables.getcolumn(value_set, keys(value_set)[1])
            @test PSRI.Tables.columnnames(new_value_st) ==
                  PSRI.Tables.columnnames(value_set)
        end
    end
end
