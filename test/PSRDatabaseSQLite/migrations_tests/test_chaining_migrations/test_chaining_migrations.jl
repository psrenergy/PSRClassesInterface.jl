...
function test_applying_migrations_from_a_certain_point()
    path_migrations_directory = joinpath(@__DIR__, "migrations")
    db = SQLite.DB()
    PSRDatabaseSQLite.apply_migrations!(db, path_migrations_directory, 1, 3, :up)
    PSRDatabaseSQLite.apply_migrations!(db, path_migrations_directory, 3, 1, :down)
    @test PSRDatabaseSQLite.db_is_empty(db)
    db = SQLite.DB()
    PSRDatabaseSQLite.apply_migrations!(db, path_migrations_directory, 1, 2, :up)
    db = SQLite.DB()
    PSRDatabaseSQLite.apply_migrations!(db, path_migrations_directory, 1, 3, :up)
    PSRDatabaseSQLite.apply_migrations!(db, path_migrations_directory, 3, 2, :down)
    PSRDatabaseSQLite.apply_migration!(db, path_migrations_directory, 1, :down)
    @test PSRDatabaseSQLite.db_is_empty(db)
    return nothing
end
