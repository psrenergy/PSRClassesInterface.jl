...
function test_invalid_database_without_configuration_table()
    ...
    @test_throws ErrorException OpenSQL.create_empty_db_from_schema(db_path, path_schema; force = true)
    rm(db_path)
    return nothing
end
...
function test_invalid_duplicated_collection_definition()
    ...
    @test_throws SQLite.SQLiteException OpenSQL.create_empty_db_from_schema(db_path, path_schema; force = true)
    rm(db_path)
    return nothing
end
