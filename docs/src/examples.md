...
```
## Determining the value of a certain demand
In this example we will read demand segments and obtain the value of a target demand. The first step is to read the study data:
```@example seg_by_dem
import PSRClassesInterface
const PSRI = PSRClassesInterface
...
```
Now, we can read the demand segments and the map between demands and demand segments, and then obtain the value of a target demand:
```@example seg_by_dem
